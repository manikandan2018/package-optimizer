...
public class PackageSpecification {
    ...
    public enum FindBestPackageStrategy {
        OPTIMIZED, BRUTE_FORCE, COMBIOPTIMIZE
    }
    ...
    /**
     * Finds the best package using the classical COMBIOPTIMIZE algorithm.
     */
    private Optional<Package> combinatorialOptimization() {
        Product[] productsArray = products.toArray(new Product[0]);
        BitSet max = new CombinatorialOptimization(productsArray).findMax(getMaxWeight());
